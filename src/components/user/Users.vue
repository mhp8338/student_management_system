            ...
            return {
                ...
                addFormRules: {
                    ...
                    password: [
                        ...
                        {min: 6, max: 15, message: '请密码的长度在6到15个之间', trigger: 'blur'}
                    ],
                    email: [
        ...
        methods: {
            ...
            editUserInfo() {
                this.$refs.editFormRef.validate(async valid => {
                    if (!valid) return
                    //发起修改用户信息请求
                    const {data: res} = await this.$http.put('edituserinfobyid/' + this.editForm.id, {
            ...
            async saveRoleInfo() {
                ...
                const {data: res} = await this.$http.put(`putusers/${this.userInfo.id}/role`, {rid: this.selectedRoleId});
                if (res.meta.status !== 200) {
